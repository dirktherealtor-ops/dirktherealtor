...
class Simulator:
    ...
    def keyword_check(self, text: str) -> None:
        lower = text.lower()
    ...
    def client_reply(self, text: str) -> str:
        lower = text.lower()
        seller = self.scenario

        if self.turn == 0:
            if '?' not in text:
                return f"Before I say much more, what would you want to know about my situation in {seller.neighborhood}?"
            if any(w in lower for w in ['why', 'moving', 'timeline', 'when']):
                return f"I'm selling because {seller.motivation}, and I would ideally like a plan that fits a {seller.urgency} timeline."
            if any(w in lower for w in ['concern', 'worry', 'important']):
                return f"Honestly, my biggest concern is {seller.concern}."
            return f"I'm open to talking, but I want to know how you would handle a seller like me who is {seller.personality}."

        if any(w in lower for w in ['market', 'price', 'pricing', 'analysis', 'comps']):
            return f"What do you think this home could realistically sell for? For context, {seller.price_hint}."
        ...
        if any(w in lower for w in ['as-is', 'repairs', 'prep', 'staging', 'showings']):
            return f"I like the sound of options, but I still worry about {seller.concern}."
    ...
    def final_assessment(self) -> Tuple[str, str, int]:
        ...
        if total >= 20:
            ...
            verdict = 'Lead Lost'
            feedback = (
                'The seller did not hear enough tailored value yet. Slow down, ask better questions, reflect their concerns, '
                'and confidently ask for an appointment.'
            )
...
def coach_note(delta: Dict[str, int], sim: Simulator) -> str:
    ...
    if delta['value'] == 0:
        notes.append('Add value around pricing, strategy, communication, marketing, or timeline.')
...
def submit_response(user_text: str) -> None:
    ...
    if sim.turn >= sim.max_turns:
        sim.history.append(('client', sim.scenario.objection))
    ...
    sim.history.append(('client', sim.client_reply(text)))
...
def render_header(sim: Simulator) -> None:
    ...
    with center:
        ...
        st.markdown(
            "<p class='hero-sub'>Mobile-friendly seller conversation trainer for mentees. Practice discovery, value, objections, and the close.</p>",
            unsafe_allow_html=True,
        )
        st.markdown(
    ...
    with right:
        dirk = load_image(DIRK_PATH)
        if dirk:
            st.image(dirk, use_container_width=True)


def render_quick_stats(sim: Simulator) -> None:
...
def render_scenario_card(sim: Simulator) -> None:
    seller = sim.scenario
    with st.expander('Seller scenario + hidden coaching notes', expanded=False):
        st.write(f"**Seller:** {seller.seller_name}")
        st.write(f"**Property:** {seller.bedrooms} BR / {seller.bathrooms} BA {seller.home_type} in {seller.neighborhood}")
        st.write(f"**Condition:** {seller.condition}")
        st.write(f"**Personality:** {seller.personality}")
        st.write(f"**Urgency:** {seller.urgency}")
        st.write(f"**Concern:** {seller.concern}")
        st.write(f"**Hidden need:** {seller.hidden_need}")
        st.write(f"**Likely objection:** {seller.objection}")
        st.write(f"**Target themes:** {', '.join(seller.must_hit_keywords)}")


def render_scoreboard(sim: Simulator) -> None:
...
def main() -> None:
    ...
    def _send_message():
        message = st.session_state.get('draft_text', '').strip()
        if message:
    ...
    with b1:
        st.button('Send', type='primary', use_container_width=True, on_click=_send_message)
    with b2:
        st.button('Sample', use_container_width=True, on_click=_load_sample)
    with b3:
        st.button('Score now', use_container_width=True, on_click=_score_now)
